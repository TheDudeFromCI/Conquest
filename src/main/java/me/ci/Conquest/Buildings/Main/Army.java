...
public class Army extends Building
{
	...
	public final PathBuilder path = new PathBuilder()
	{
		public ArrayList<Node> getOpenPoints(Node o)
		{
			...
			n7.setMoveCost(14);
			n8.setMoveCost(14);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n1.getX(), n1.getY())
						.isClaimed())
				nodes.add(n1);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n2.getX(), n2.getY())
						.isClaimed())
				nodes.add(n2);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n3.getX(), n3.getY())
						.isClaimed())
				nodes.add(n3);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n4.getX(), n4.getY())
						.isClaimed())
				nodes.add(n4);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n5.getX(), n5.getY())
						.isClaimed())
				nodes.add(n5);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n6.getX(), n6.getY())
						.isClaimed())
				nodes.add(n6);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n7.getX(), n7.getY())
						.isClaimed())
				nodes.add(n7);
			if (!PlotMg	.getPlotAt(nwc.getWorld(), n8.getX(), n8.getY())
						.isClaimed())
				nodes.add(n8);
			final int h = PlotMg.getPlotAt(nwc)
								.getHeight();
			for (Node n : nodes)
			{
				n.setMoveCost(n.getMoveCost() * (Math.abs(h - PlotMg.getPlotAt(nwc.getWorld(), n.getX(), n.getY())
																	.getHeight())
						+ 1));
			}
			return nodes;
	...
	{
		return getType().getId() + ";" + getNorthWestCourner().getX() + "," + getNorthWestCourner().getZ() + ";"
				+ realtype.getId() + ";" + hp + ";" + formation.getId() + ";" + selected + ";" + stamina + ";" + lvl
				+ ";" + (pf == null ? "0" : pf	.getEnd()
												.getX()
						+ "," + pf	.getEnd()
									.getY())
				+ ";" + nullified + ";" + wizard + ";" + commander + ";"
	...
	public void loadSaveStats(String code)
	{
		try
		{
			...
			if (!s[8].equals("0"))
				setMoveGoal(Integer.valueOf(s[8].split(",")[0]), Integer.valueOf(s[8].split(",")[1]));
	...
	{
		if (!type.isSiegeWeapon())
			ConquestTextures.getTexture(BuildingType.ARMY, lvl)
							.buildAt(nwc, this);
		else
		{
			int id;
			if (type == ArmyType.ANTIMAGIC_ONAGER)
				id = 8;
			else if (type == ArmyType.BALLISTA)
				id = 12;
			else if (type == ArmyType.BATTERING_RAM)
				id = 16;
			else if (type == ArmyType.BRIDGEMAKER)
				id = 20;
			else if (type == ArmyType.CATAPULT)
				id = 24;
			else if (type == ArmyType.COW_TOSSER)
				id = 28;
			else if (type == ArmyType.MANGONAL)
				id = 32;
			else if (type == ArmyType.SIEGETOWER)
				id = 36;
			else
				id = 40;
			id += direction.getId();
			ConquestTextures.getTexture(BuildingType.ARMY, id)
							.buildAt(nwc, this);
		}
	}
	...
	public String[] getInfo()
	{
		...
		s[7] = ChatColor.DARK_AQUA + "Move Target ............ " + ChatColor.GRAY + (pf != null ? pf.getEnd()
																									.getX()
				+ ", " + pf	.getEnd()
							.getY()
				: "Not Moving");
		s[8] = ChatColor.DARK_AQUA + "Can Attack ............. " + ChatColor.GRAY + canAttack();
		s[9] = ChatColor.DARK_AQUA + "Direction .............. " + ChatColor.GRAY + direction	.name()
																								.toLowerCase();
	...
	{
		return stamina / (double) getMaxStamina() * 100;
	}

	public boolean isCamping()
	{
		return formation == ArmyFormation.P;
	}
	...
	{
		int h = Math.abs(to.getHeight() - PlotMg.getPlotAt(nwc)
												.getHeight())
				+ 1;
		if (flytime > -1)
			takestamina = false;
		if (takestamina && getStamina() >= h)
			setStamina(getStamina() - h);
		else
			return;
		kingdom.removeBuilding(Army.this, false, true, false);
		nwc = to.getChunk();
		List<Building> builds;
		if (buildings.containsKey(kingdom))
			builds = buildings.get(kingdom);
		else
			builds = new LinkedList<Building>();
		...
		updateDirection();
		tellWatchTowers("There is army movement near your kingdom!", null,
				"The army units are from " + kingdom.getName() + ", owned by " + kingdom.getOwner() + ".");
		updateGraphics(false);
	}
	...
	private boolean checkMoat()
	{
		if (attacking == null)
			return true;
		if (attacking.getType() != BuildingType.MOAT)
			return true;
		return false;
	}
	...
	private void move()
	{
		...
		try
		{
			if (PlotMg	.getPlotAt(getWorld(), pf	.getEnd()
													.getX(),
								pf	.getEnd()
									.getX())
						.isClaimed())
	...
	public void attack(final double percent, final boolean cost, final boolean message, final boolean takeblessing)
	{
		...
		if (attacking instanceof Army)
		{
			...
			if (attacking	.getNorthWestCourner()
							.getZ() < nwc.getZ()
					&& attacking.getNorthWestCourner()
								.getX() == nwc.getX())
			{
				if (!army	.getFormation()
							.usesSide(Direction.NORTH))
					weakside = true;
			}
			else if (attacking	.getNorthWestCourner()
								.getZ() > nwc.getZ()
					&& attacking.getNorthWestCourner()
								.getX() == nwc.getX())
			{
		...
		if (weakside)
			damage *= 2;
		double h = PlotMg	.getPlotAt(nwc)
							.getHeight()
				- PlotMg.getPlotAt(attacking.getNorthWestCourner())
						.getHeight();
		h /= 5;
		h++;
		h = Math.min(h, 2);
		...
		if (attacking.hasDivineProtection() && illusiontime > -1)
			attacking.damage(kingdom, 0);
		if (attacking.hasTrap())
			damage(kingdom, damage);
		else if (illusiontime > -1 && attacking instanceof Army && ((Army) attacking).speartime > -1)
		{
		...
		else if (illusiontime == -1)
		{
			...
			tellWatchTowers("Your kingdom is under attack!", attacking.getKingdom(),
					"The attacker is from the kingdom of " + kingdom + ", owned by " + kingdom.getOwner() + ".");
		}
		else
		{
			if (hp == 1)
		...
		{
			if (!(attacking instanceof Army) && !(attacking instanceof Land) && attacking	.getKingdom()
																							.getMorale() > 0)
				attacking	.getKingdom()
							.removeMorale(1);
			attacking = null;
			if (kingdom.getMorale() < 100)
				kingdom.addMorale(1);
		}
		if (cost)
	...
	private void stealGold(final int amount)
	{
		...
		tellWatchTowers("Gold was stolen from your kingdom!", attacking.getKingdom(),
				"The pillagers are from the kingdom of " + kingdom + ", owned by " + kingdom.getOwner() + ".");
	}

	private void dropTnt()
	{
		final double x = Math.random() * 16 * attacking.getLength() + attacking	.getNorthWestCourner()
																				.getBlock(0, 0, 0)
																				.getX();
		final double z = Math.random() * 16 * attacking.getLength() + attacking	.getNorthWestCourner()
																				.getBlock(0, 0, 0)
																				.getZ();
		final double y = 90 + PlotMg.getPlotAt(attacking.getNorthWestCourner())
									.getHeight();
		final Location l = new Location(attacking.getWorld(), x, y, z);
		TNTPrimed tnt = attacking	.getWorld()
									.spawn(l, TNTPrimed.class);
		tnt.setVelocity(new Vector(0, -500, 0));
		tnt.setFuseTicks(20);
		Sound s;
		final int r = (int) (Math.random() * 3);
		if (r == 0)
			s = Sound.BLOCK_ANVIL_USE;
		else if (r == 1)
			s = Sound.BLOCK_ANVIL_LAND;
		else
			s = Sound.BLOCK_ANVIL_BREAK;
		for (Player p : Bukkit.getOnlinePlayers())
		{
			p.playSound(l, s, 1.0f, 1.0f);
		}
	}
	...
	public boolean isTargetInRange()
	{
		...
		if (attacking	.getNorthWestCourner()
				...
				&& attacking.getNorthWestCourner()
							.getX() == nwc.getX())
		{
			if (formation.usesSide(Direction.NORTH) && nwc.getZ() - attacking	.getNorthWestCourner()
																				.getZ() <= type.getRange())
				direction = true;
		}
		else if (attacking	.getNorthWestCourner()
				...
				&& attacking.getNorthWestCourner()
							.getX() == nwc.getX())
		{
			if (formation.usesSide(Direction.SOUTH) && attacking.getNorthWestCourner()
																.getZ()
					- nwc.getZ() <= type.getRange())
				direction = true;
		}
		else if (attacking	.getNorthWestCourner()
							.getX() > nwc.getX()
				&& attacking.getNorthWestCourner()
							.getZ() == nwc.getZ())
		{
			if (formation.usesSide(Direction.EAST) && attacking	.getNorthWestCourner()
																.getX()
					- nwc.getX() <= type.getRange())
				direction = true;
		}
		else
		{
			if (formation.usesSide(Direction.WEST) && nwc.getX() - attacking.getNorthWestCourner()
																			.getX() <= type.getRange())
				direction = true;
		}
		direction = direction && Math.abs(PlotMg.getPlotAt(attacking.getNorthWestCourner())
												.getHeight()
				- PlotMg.getPlotAt(nwc)
						.getHeight())
				/ 5 < type.getRange();
		return direction;
	}
	...
	public int getMaxHp()
	{
		return ArmyType.getMaxAmount(lvl);
	}
	...
	{
		if (isCamping())
			return "No, this troop is camping.";
		if (getStamina() <= 0)
			return "No, this troop has no stamina.";
		if (!isAttacking())
			return "No, This troop has no target.";
		if (!isTargetInRange())
			return "No, target out of range.";
		if (!kingdom.isOwnerOnline())
			return "No, the owner of this troop's kingdom is offline.";
		if (attacking.isBuildingOrUpgrading())
			return "No, the target is being build ot upgraded.";
		if (!isOffical())
			return "No, this troop is corrupt.";
		if (!attacking.isOffical())
			return "No, the target is corrupt.";
		if (!checkMoat())
			return "No, the target is water type.";
		if (kingdom.getResourceLevels(ResourceType.ARMS) < (int) (hp * 0.1))
			return "No, not enough arms to attack.";
		if (!attacking	.getKingdom()
						.isOwnerOnline())
			return "No, the owner of the target's kingdom is offline.";
		return "Yes";
	}
	...
	{
		if (protecttime > -1 && protector.isOffical())
		{
			amount /= 2;
	...
	public void delete(final boolean a, final boolean b)
	{
		new Thread(new Runnable()
		{
			public void run()
			{
				...
				if (PlayerMg.getSelectedArmy(kingdom.getOwner()) == Army.this)
					PlayerMg.setSelectedArmy(kingdom.getOwner(), null);
				synchronized (armies)
				{
					armies.remove(this);
				}
	...
	private void updateDirection()
	{
		...
		final double d = Math.atan2(nwc.getZ() - attacking	.getNorthWestCourner()
															.getZ(),
				nwc.getX() - attacking	.getNorthWestCourner()
										.getX())
				* 180 / Math.PI % 360;
		if (45 <= d && d <= 135)
	...
	private void tellWatchTowers(final String message, final Kingdom king, final String advanced)
	{
		if (king == null)
		{
			...
			messageTower(message, king, advanced);
	}
	...
	private void messageTower(final String message, final Kingdom k, final String advanced)
	{
		...
		for (int x = -10; x <= 10; x++)
		{
			for (int z = -10; z <= 10; z++)
			{
				...
				if (plot.isKingdomPlot() && plot.getKingdom() == k)
				{
					tell = true;
					break checker;
				}
	...
	private Building checkForNearbyThreats(final boolean assign)
	{
		...
		for (int x = -8; x <= 8; x++)
		{
			...
			{
				plot = PlotMg.getPlotAt(getWorld(), x + nwc.getX(), z + nwc.getZ());
				if (plot.isKingdomPlot() && plot.getKingdom() != kingdom && !plot	.getKingdom()
																					.isRuins()
						&& plot	.getKingdom()
								.isOwnerOnline())
				{
					if (plot.getBuilding()
						...
						list.add(plot.getBuilding(), (int) ((9 - getDistance(plot.getBuilding())) * 10));
				}
			}
		}
	...
	private boolean isWithinRange()
	{
		if (attacking == null)
			return false;
		final int rangex = Math.abs(attacking	.getNorthWestCourner()
												.getX()
				- nwc.getX());
		final int rangez = Math.abs(attacking	.getNorthWestCourner()
												.getZ()
				- nwc.getZ());
		if (rangex <= type.getRange() && rangez <= type.getRange())
			return Math.abs(PlotMg	.getPlotAt(attacking.getNorthWestCourner())
									.getHeight()
					- PlotMg.getPlotAt(nwc)
							.getHeight())
					/ 5 < type.getRange();
	...
	private boolean chooseBestAttackPosition()
	{
		if (attacking == null)
			return false;
		final int baseX = attacking	.getNorthWestCourner()
									.getX();
		final int baseZ = attacking	.getNorthWestCourner()
									.getZ();
		...
		{
			for (int o = 0; o < attacking	.getType()
											.getLength();
					o++)
			{
	...
	{
		return isWithinRange() && isProperAngle();
	}

	private int getPriority(final Plot plot, final Plot myP, final Plot urP, final Direction d)
	{
		if (!isHeightRange(plot.getHeight(), urP.getHeight()))
			return -1;
		int heightBonus = plot.getHeight() - urP.getHeight();
		if (wizard)
			heightBonus = Math.max(heightBonus, 0);
		final int walk = isPlotEasilyReachable(plot);
		if (walk == -1)
			return -1;
		final boolean flank = (attacking instanceof Army ? !((Army) attacking)	.getFormation()
																				.usesSide(d)
				: false);
	...
	private boolean isHeightRange(final int h1, final int h2)
	{
		return Math.abs(h1 - h2) / 5 < type.getRange();
	}
			...
			public void run()
			{
				...
				if (stamina < 1)
				{
					if (formation != ArmyFormation.P)
						setFormation(ArmyFormation.P);
					return;
				}
				if (isMoving())
				{
					if (stamina >= 10 && formation == ArmyFormation.P)
						setFormation(ArmyFormation.A);
					return;
				}
				...
				if (attacking != null)
				{
					...
					if (!attacking	.getKingdom()
									.isOwnerOnline())
						attacking = null;
					if (getDistance(attacking) > 15)
						attacking = null;
				}
				// Check if theres an enemy
				if (attacking == null && checkForNearbyThreats(true) == null)
				...
				if (formation == ArmyFormation.P && stamina < 8)
					return;
				// Make sure target is in prefect position
				if (!chooseBestAttackPosition())
					checkForNearbyThreats(true);
				// Check sides
	...
	{
		return !isCamping() && getStamina() > 0 && isAttacking()
				&& (isTargetInRange() || (ignoreangle && isWithinRange())) && kingdom.isOwnerOnline()
				&& !attacking.isBuildingOrUpgrading() && isOffical() && attacking.isOffical()
				&& kingdom.getResourceLevels(ResourceType.ARMS) >= (int) (hp * 0.1) && checkMoat()
				&& attacking.getKingdom()
							.isOwnerOnline();
	}
	...
	public void slayDragons()
	{
		...
		for (int x = -3; x <= 3; x++)
		{
			for (int z = -3; z <= 3; z++)
			{
				...
				if (plot.isKingdomPlot() && plot.getKingdom() != kingdom && plot.getBuilding() instanceof Army
						&& ((Army) plot.getBuilding()).getArmyType() == ArmyType.DRAGON_RIDER)
				{
					attacking = plot.getBuilding();
					if (isTargetAttackable(true))
						attack(1.0, false, false, false);
				}
			}
		}
		blessed = false;
	...
	public ArmyType getRealArmyType()
	{
		return realtype;
	}

	public void setRealArmyType(final ArmyType type)
	{
		realtype = type;
	}

	public void usePower(final Kingdom k, final String advanced)
	{
		tellWatchTowers("A military unit from " + kingdom + " is using abilities against your kingdom!", k, advanced);
	}

	public double getDistance(final Building t)
	{
		return Math.sqrt(Math.pow(t	.getNorthWestCourner()
									.getX()
				- nwc.getX(), 2) + Math.pow(
						t	.getNorthWestCourner()
							.getZ() - nwc.getZ(),
						2));
	}

	public double getDistance(final Building t, final Plot p)
	{
		return Math.sqrt(Math.pow(t	.getNorthWestCourner()
									.getX()
				- p.getX(), 2) + Math.pow(
	...
	public void fly()
	{
		flytime = System.currentTimeMillis() + 300000;
	}
	...
	public void slay()
	{
		slaytime = System.currentTimeMillis() + 180000;
	}

	public boolean isUnderIllusion()
	{
		return illusiontime > -1;
	}

	public void illusion()
	{
		illusiontime = System.currentTimeMillis() + 60000;
	}

	public boolean isProtected()
	{
		return protecttime > -1;
	}
	...
	public void takeBlessing()
	{
		blessed = false;
	}

	public boolean isInSpearMode()
	{
		return speartime > -1;
	}

	public void spearMode()
	{
		speartime = System.currentTimeMillis() + 240000;
	}

	public void pillage()
	{
		pillagetime = System.currentTimeMillis() + 300000;
	}

	public boolean isPillaging()
	{
		return pillagetime > -1;
	}

	public void addHeldArmy(final Army a)
	{
		heldarmies.add(new HeldArmy(a));
	}

	public ArrayList<HeldArmy> getHeldArmies()
	{
		return heldarmies;
	}

	public void removeHeldArmy(final HeldArmy a)
	{
		heldarmies.remove(a);
	}
					...
					synchronized (armies)
					{
						for (Army a : armies)
						{
							try
							{
								...
								{
									if (a.attacking != null && !a.attacking.isOffical())
										a.attacking = null;
									a.prepareAttack();
									if (a.isTargetAttackable(false))
										a.attack(1.0, true, true, true);
									if (!a.isCamping() && a.getStamina() > 0 && a.isMoving())
										a.move();
									a.actiontime =
											(long) (System.currentTimeMillis() + a.type.getReload(a.lvl) * 1000L);
								}
								if (a.reverttime != -1 && a.reverttime <= System.currentTimeMillis())
								{
									a.reverttime = -1;
									a.type = a.realtype;
									a.updateGraphics(false);
								}
								if (a.flytime != -1 && a.flytime <= System.currentTimeMillis())
									a.flytime = -1;
								if (a.illusiontime != -1 && a.illusiontime <= System.currentTimeMillis())
									a.illusiontime = -1;
								if (a.protecttime != -1 && a.protecttime <= System.currentTimeMillis())
									a.protecttime = -1;
								if (a.speartime != -1 && a.speartime <= System.currentTimeMillis())
									a.speartime = -1;
								if (a.pillagetime != -1 && a.pillagetime <= System.currentTimeMillis())
									a.pillagetime = -1;
								if (a.slaytime != -1 && a.slaytime <= System.currentTimeMillis())
									a.slaytime = -1;
								else if (a.slaytime > -1)
									a.slayDragons();
							}
							catch (final Exception exception)
							{
