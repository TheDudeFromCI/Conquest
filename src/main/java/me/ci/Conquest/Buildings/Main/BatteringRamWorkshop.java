...
public class BatteringRamWorkshop extends Building implements TroopConvertable
{
	...
	public BatteringRamWorkshop(Chunk nwc, Kingdom kingdom)
	{
		super(nwc, kingdom);
	...
	public BatteringRamWorkshop(Kingdom kingdom, String code, Chunk nwc, long id)
	{
	...
	public String getSaveStats()
	{
		...
		return getType().getId() + ";" + getNorthWestCourner().getX() + "," + getNorthWestCourner().getZ() + ";"
				+ getHp() + ";" + getLevel() + ";" + converttime + ";" + amount + ";"
	...
	public void loadSaveStats(String code)
	{
		...
		String[] s1 = s[6].split(",");
		to = getCenter().getWorld()
						.getChunkAt(Integer.valueOf(s1[0]), Integer.valueOf(s1[1]));
	}

	public void updateGraphics(boolean threadsleep)
	{
		ConquestTextures.getTexture(getType(), lvl)
						.buildAt(nwc, this);
	}
	...
	public String[] getInfo()
	{
		...
		s[3] = ChatColor.DARK_AQUA + "Information ............ " + ChatColor.AQUA + getNewMessages() + ChatColor.GRAY
				+ " New, " + ChatColor.AQUA + getOldMessages() + ChatColor.GRAY + " Unanswered Messages";
		...
		s[6] = ChatColor.DARK_AQUA + "Status ................. " + ChatColor.GRAY + toTitleCase(getStatus()	.name()
																											.toLowerCase());
		...
		s[11] = ChatColor.DARK_AQUA + "Conversion Percent ..... " + ChatColor.AQUA
				+ (int) ((totalconverttime - converttime) / (double) totalconverttime * 100) + "%";
		s[12] = ChatColor.DARK_AQUA + "Conversion Amount ...... " + ChatColor.AQUA + amount;
		s[13] = ChatColor.DARK_AQUA + "Conversion Location .... " + ChatColor.AQUA
				+ (to == null ? "0,0" : to.getX() + "," + to.getZ());
	...
	public void convert(int amount, Chunk to)
	{
		...
		updateStatus();
		final Plot plot = PlotMg.getPlotAt(to);
		for (int x = 0; x < 16; x++)
		{
			for (int z = 0; z < 16; z++)
			{
				if (x == 0 || x == 15 || z == 0 || z == 15)
					to	.getBlock(x, plot.getHeight() + 3, z)
	...
	public void removeTime(final int i)
	{
		new Thread(new Runnable()
		{
			public void run()
			{
				...
				if (converttime <= 0)
				{
					if (!PlotMg	.getPlotAt(to)
								.isKingdomPlot())
						new Army(to, kingdom, amount, getConvertTo(), lvl);
					else
					{
						kingdom.addVillagers(amount);
						amount = 0;
						to = null;
						Bukkit	.getPlayer(getKingdom().getOwner())
								.sendMessage(ChatColor.RED + "Could not place military unit, area taken!");
					}
					updateStatus();
				}
			}
