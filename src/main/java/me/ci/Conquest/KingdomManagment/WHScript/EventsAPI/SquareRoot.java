...
public class SquareRoot extends Action{
	public SquareRoot(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(Math.sqrt(((NumberClass)args.getInputs()[0]).getState())));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}