...
import me.ci.Conquest.KingdomManagment.WHScript.WraithavenScript.KingdomClass;
import me.ci.Conquest.KingdomManagment.WHScript.WraithavenScript.PrimitiveType;

public class GetKingdomMorale extends Action{
	public GetKingdomMorale(final WraithavenScript whs, final ActionLineFormat format, final int line){
		...
		try{
			...
			if(args.getInputs()[0].getPrimitiveType()!=PrimitiveType.KINGDOM)throw new Exception("Incorrect variable type: "+args.getInputs()[0].getPrimitiveType()+".");
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(((KingdomClass)args.getInputs()[0]).getState().getMorale()));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}