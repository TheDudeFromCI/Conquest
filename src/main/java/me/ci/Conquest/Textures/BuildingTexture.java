...
public class BuildingTexture
{
	...
	{
		if (WhCommunity.debug)
			WhCommunity.printDebug();
		...
		for (int i = 0; i < s1.length; i++)
		{
			...
			for (int i1 = 0; i1 < s2.length; i1++)
			{
				...
				{
					if (!s2[i1].isEmpty())
						this.blocks[i][i1] = new BlockInfo(s2[i1]);
				}
	...
	private void draw(final Chunk nwc, final Building building)
	{
		...
		try
		{
			...
			for (int cx = 0; cx < s; cx++)
			{
				...
				{
					final Chunk chunk1 = PlotMg	.getPlotAt(nwc.getWorld(), cx + chunkoffx, cz + chunkoffz)
												.getChunk();
					Bukkit	.getScheduler()
							.runTask(WhCommunity.plugin, new Runnable()
							{
			...
			for (int i = 0; i < 91; i++)
			{
				...
				for (int c = 0; c < blocks.length; c++)
				{
					...
					Chunk chunk = nwc	.getWorld()
										.getChunkAt(chunkx, chunkz);
					int basex = chunk	.getBlock(0, 0, 0)
										.getX();
					int basez = chunk	.getBlock(0, 0, 0)
										.getZ();
					try
					{
						boolean die = false;
							...
							try
							{
								for (int x = 0; x < 16; x++)
								{
									for (int z = 0; z < 16; z++)
									{
										...
										// {
										// if (blocks[c10][l10[c10]].id == 63 || blocks[c10][l10[c10]].id == 54
										...
										// }
										// if (blocks[c10][l10[c10]].getId() == 7)
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(1, (byte) 0, false);
										// else if (building != null)
										// {
										// if (blocks[c10][l10[c10]].getId() == 121)
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(35, (building .getKingdom()
										// .isRuins() ? (byte) 0
										// : f.getPrimaryColor()),
										// false);
										// else if (blocks[c10][l10[c10]].getId() == 19)
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(35, (building .getKingdom()
										// .isRuins() ? (byte) 0
										// : f.getSecondaryColor()),
										// false);
										// else if (blocks[c10][l10[c10]].getId() == 120)
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(35, (building .getKingdom()
										// .isRuins() ? (byte) 0
										// : f.getTertiaryColor()),
										// false);
										// else if (blocks[c10][l10[c10]].getId() == 7)
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(1, (byte) 0, false);
										// else if (blocks[c10][l10[c10]].getId() == 99)
										// {
										// if (!building .getKingdom()
										// .isRuins())
										// {
										...
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(0, (byte) 0, false);
										// try
										// {
										// WhCommunity.threads++;
										// Bukkit .getScheduler()
										// .runTaskLater(WhCommunity.plugin, new Runnable()
										// {
										// public void run()
										// {
										// try
										// {
										// f.draw(w.getBlockAt(flagplacex,
										// flagplacey, flagplacez));
										// }
										// catch (Exception exception)
										// {
										// exception.printStackTrace();
										...
										// else
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setType(Bukkit.createBlockData(""));
										// .setTypeIdAndData(blocks[c10][l10[c10]].getId(),
										// blocks[c10][l10[c10]].getData(), false);
										// }
										// else
										// w .getBlockAt(x + basex10, i10, z + basez10)
										// .setTypeIdAndData(blocks[c10][l10[c10]].getId(),
					...
					Bukkit	.getScheduler()
							.runTaskLater(WhCommunity.plugin, run, (int) (i * 1.5 + 5));
				}
				if (i == 10 && building != null)
				{
					if (!building.isBuildingOrUpgrading() || building instanceof Army)
	...
	private synchronized static void uploadChunks(final List<Chunk> chunks, final BuildingTexture text,
			final boolean load)
	{
		for (Chunk c : chunks)
		{
			...
			if (updated.containsKey(c))
				textures = updated.get(c);
			else
				textures = new LinkedList<BuildingTexture>();
			if (load)
