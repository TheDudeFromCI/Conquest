...
{
	private static Map<String, Map<Integer, BuildingTexture>> builds =
			new HashMap<String, Map<Integer, BuildingTexture>>();
	...
	private static void load(BuildingType type, int l)
	{
		...
		if (builds.containsKey(type.getName()))
			b = builds.get(type.getName());
		else
			b = new HashMap<Integer, BuildingTexture>();
	...
	public static void save(Chunk NWCourner, BuildingType type, int lvl)
	{
		...
		for (int x = 0; x < type.getLength(); x++)
		{
			...
			{
				Chunk c = NWCourner	.getWorld()
									.getChunkAt(x + NWCourner.getX(), z + NWCourner.getZ());
		...
		while (pointlist.startsWith(";"))
			pointlist = pointlist.substring(1);
		Save.set("Resources", "Building Waypoints", type.getName(), pointlist);
		if (WhCommunity.CONFIG_Save_Conquest_To_Ram)
			load(type, lvl);
	}

	private static String saveChunk(Chunk chunk, double total, Percent done, int lvl, String name,
			List<Location> waypoints, int offchunkx, int offchunkz)
	{
		if (WhCommunity.debug)
			WhCommunity.printDebug();
		String out = "";
		for (int y = 0; y < 90; y++)
		{
			for (int x = 0; x < 16; x++)
			{
				for (int z = 0; z < 16; z++)
				{
					...
					// else{
					// if(b.getTypeId()==122){
					// waypoints.add(new Location(chunk.getWorld(), offchunkx*16+x, y,
					// offchunkz*16+z));
					// out+=",";
					// }else{
	...
	private static BuildingTexture getTexture(String building, int lvl)
	{
		...
		if (WhCommunity.CONFIG_Save_Conquest_To_Ram)
		{
			if (builds.containsKey(building))
			{
				...
				if (build.containsKey(lvl))
					return build.get(lvl);
			}
		}
		else
		{
