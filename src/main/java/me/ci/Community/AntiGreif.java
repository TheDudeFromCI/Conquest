...
public class AntiGreif
{
	...
	public static void yellAtPlayer(Location l, Player p)
	{
		...
		{
			l	.getWorld()
				.playEffect(l, Effect.MOBSPAWNER_FLAMES, 0);
			l	.getWorld()
				.playEffect(l, Effect.EXTINGUISH, 0);
		}
	...
	// WorldSettings settings = WhCommunity.getWorldSettings(e .getPainting()
	// .getWorld()
	// .getName());
	// if (reason.equals(RemoveCause.FIRE) && !settings.CONFIG_Fire_Spread)
	// e.setCancelled(true);
	// }
	...
	public static void e(BlockPlaceEvent e)
	{
		...
		{
			if (p.hasPermission("whc.cmd.kingdom.savechunk") && KingdomCommand.flagsPlanted.containsKey(p.getName())
	...
	public static void e(BlockBurnEvent e)
	{
		Location l = e	.getBlock()
						.getLocation();
		WorldSettings settings = WhCommunity.getWorldSettings(l	.getWorld()
																.getName());
		if (!settings.CONFIG_Fire_Spread)
			e.setCancelled(true);
	}
	...
	public static void e(BlockIgniteEvent e)
	{
		...
		else if (!settings.CONFIG_Fire_Spread)
			e.setCancelled(true);
	}

	public static void e(BlockFromToEvent e)
	{
		Plot p1 = PlotMg.getPlotAt(e.getBlock()
									.getChunk());
		Plot p2 = PlotMg.getPlotAt(e.getToBlock()
									.getChunk());
		WorldSettings settings = WhCommunity.getWorldSettings(e	.getBlock()
																.getWorld()
																.getName());
		if (settings.CONFIG_Plot_Barrier_Liquid_Flow)
		{
			if (p1.getX() != p2.getX() || p1.getZ() != p2.getZ() && p2.isClaimed())
			{
				if (p1.isClaimed() && !p1	.getOwner()
											.equals(p2.getOwner()))
					return;
				else
					e.setCancelled(true);
	...
	{
		WorldSettings settings = WhCommunity.getWorldSettings(e	.getLocation()
																.getWorld()
																.getName());
		if (!settings.CONFIG_Explosions)
			e	.blockList()
				.clear();
	}
	...
	public static void e(CreatureSpawnEvent e)
	{
		...
		{
			if (!settings.CONFIG_Spawn_Mobs_Plugin)
				e.setCancelled(true);
		}
		else
		{
			...
			if (mob.equals(EntityType.BAT))
				hostile = false;
			else if (mob.equals(EntityType.CHICKEN))
				hostile = false;
			else if (mob.equals(EntityType.COW))
				hostile = false;
			else if (mob.equals(EntityType.IRON_GOLEM))
				hostile = false;
			else if (mob.equals(EntityType.MUSHROOM_COW))
				hostile = false;
			else if (mob.equals(EntityType.OCELOT))
				hostile = false;
			else if (mob.equals(EntityType.PIG))
				hostile = false;
			else if (mob.equals(EntityType.SHEEP))
				hostile = false;
			else if (mob.equals(EntityType.SNOWMAN))
				hostile = false;
			else if (mob.equals(EntityType.SQUID))
				hostile = false;
			else if (mob.equals(EntityType.VILLAGER))
				hostile = false;
			else if (mob.equals(EntityType.WOLF))
				hostile = false;
			if (hostile)
			{
				if (!settings.CONFIG_Spawn_Mobs_Naturally_Hostile)
					e.setCancelled(true);
			}
			else if (!settings.CONFIG_Spawn_Mobs_Naturally_Peaceful)
				e.setCancelled(true);
		}
	}
		...
		try
		{
			...
			if (b != null)
			{
				...
				if (!PlayerMg.hasBuildPermission(l, p))
				{
					...
					// && bid == 54 || bid == 84 || bid == 92 || bid == 120 || bid == 138 || bid ==
					// 145
					// || bid == 26 || bid == 96 || bid == 60 || bid == 23 || bid == 25 || bid ==
					// 107 || bid == 61
					// || bid == 122)
					// {
					// e.setCancelled(true);
								...
								if (a.equals(Action.LEFT_CLICK_BLOCK))
								{
									if (AntiGreif.channels.containsKey(p.getName()))
									{
										...
										if (c.getId() != id)
										{
											...
											{
												if (PlayerMg.hasPermission(p, "whc.chat.join." + p	.getWorld()
																									.getName()
														+ ".*") || PlayerMg
																			.hasPermission(p,
																					"whc.chat.join." + p.getWorld()
																										.getName()
																							+ "." + id))
												{
													c.leaveChannel(p);
													...
													p.sendMessage(
															ChatColor.DARK_AQUA + "You have joined channel: " + newc);
												}
												else
													p.sendMessage(ChatColor.RED
															+ "You do not have permission to join this chat channel!");
											}
											else
												p.sendMessage(
														ChatColor.RED + "This channel does not exist for this world!");
										}
										else
											p.sendMessage(ChatColor.RED + "You are already in this channel!");
									}
									else
										p.sendMessage(ChatColor.RED
												+ "You have not selected a channel yet, so you cannot join one! Right click to scroll through channels and then pick one by left clicking.");
								}
								else if (a.equals(Action.RIGHT_CLICK_BLOCK))
								{
									if (settings.CONFIG_Chat_Channels_Limit == 0)
										p.sendMessage(ChatColor.RED + "There are no chat channels for this world!");
									else
									{
										int c = -1;
										if (AntiGreif.channels.containsKey(p.getName()))
											c = AntiGreif.channels.get(p.getName());
										c++;
										c %= settings.CONFIG_Chat_Channels_Limit;
										AntiGreif.channels.put(p.getName(), c);
										Channel channel = settings.getChannelById(c);
										p.sendMessage(ChatColor.DARK_AQUA + "Left click to join channel "
												+ ChatColor.AQUA + channel + ChatColor.DARK_AQUA + ".");
									}
								}
								e.setCancelled(true);
							}
				...
				if (item != null)
				{
					if (item.getType() == Material.INK_SAC)
					{
						if (KingdomMaker.colorflag.containsKey(p.getName()))
						{
							...
							Block flagbase = KingdomMg	.getCreateFlagLocation(b.getWorld(), flaginfo[0])
														.getBlock();
							final byte data = getWoolColor(item	.getData()
																.getData());
							if (Flag.isPartOfWool(flagbase, b, flaginfo[0]))
							{
								...
								new Thread(new Runnable()
								{
									public void run()
									{
										...
										{}
										p.sendBlockChange(b.getLocation(), Material.WHITE_WOOL, data);
									}
								}).start();
							}
						}
					...
					else if (item.getType() == Material.LAVA_BUCKET)
					{
						if (KingdomMaker.colorflag.containsKey(p.getName()))
						{
							...
							final Block flagbase = KingdomMg.getCreateFlagLocation(b.getWorld(), flaginfo[0])
															.getBlock();
							if (Flag.isPartOfWool(flagbase, b, flaginfo[0]))
							{
								...
								new Thread(new Runnable()
								{
									public void run()
									{
										...
										for (int i = 1; i < flaginfo.length; i++)
										{
											p.sendBlockChange(Flag	.getBlockAt(flaginfo, flagbase, i)
																	.getLocation(),
													Material.WHITE_WOOL, flaginfo[i]);
										}
									}
								}).start();
							}
				...
				{
					byte id = (byte) -1;
					if (Flag.isPartOfFlag(KingdomMg	.getCreateFlagLocation(b.getWorld(), (byte) 0)
													.getBlock(),
							b, (byte) 0))
						id = (byte) 0;
					else if (Flag.isPartOfFlag(KingdomMg.getCreateFlagLocation(b.getWorld(), (byte) 1)
														.getBlock(),
							b, (byte) 1))
						id = (byte) 1;
					else if (Flag.isPartOfFlag(KingdomMg.getCreateFlagLocation(b.getWorld(), (byte) 2)
														.getBlock(),
							b, (byte) 2))
						id = (byte) 2;
					else if (Flag.isPartOfFlag(KingdomMg.getCreateFlagLocation(b.getWorld(), (byte) 3)
														.getBlock(),
							b, (byte) 3))
						id = (byte) 3;
					else if (Flag.isPartOfFlag(KingdomMg.getCreateFlagLocation(b.getWorld(), (byte) 4)
														.getBlock(),
							b, (byte) 4))
						id = (byte) 4;
					if (id > -1)
	...
	public static void e(EntityDamageByEntityEvent e)
	{
		...
		{
			if (!simulateAttack((Player) attacker, victum))
				e.setCancelled(true);
		}
		else if (attacker instanceof Arrow)
		{
			var shooter = ((Arrow) attacker).getShooter();
			if (shooter != null && shooter instanceof Player)
			{
				if (!simulateAttack((Player) shooter, victum))
					e.setCancelled(true);
			}
		}
	}
	...
	public static boolean simulateAttack(Player p, Entity e)
	{
		...
		{
			Player v = (Player) e;
			WorldSettings settings = WhCommunity.getWorldSettings(e	.getWorld()
																	.getName());
			if (settings.CONFIG_Pvp_Global)
				return true;
			if (PlayerMg.hasPvpEnabled(p) && PlayerMg.hasPvpEnabled(v))
				return true;
			else
			{
				if (PlayerMg.hasPvpEnabled(p))
					p.sendMessage(ChatColor.RED + "You do not have Pvp turned on, you cannot attack that player!");
				else
					p.sendMessage(ChatColor.RED + "That player does not have Pvp turned on, you cannot attack them!");
				return false;
			}
		}
		else
		{
			...
			if (!hostile)
			{
				if (PlayerMg.hasBuildPermission(e.getLocation(), p))
					return true;
				else
				{
					p.sendMessage(
							ChatColor.RED + "You cannot attack peaceful mobs in areas you do not have permission for!");
					return false;
				}
			}
		}
	...
	public static void e(BlockPhysicsEvent e)
	{
		WorldSettings settings = WhCommunity.getWorldSettings(e	.getBlock()
																.getWorld()
																.getName());
		if (settings.CONFIG_Conquest_Enabled)
			e.setCancelled(true);
	}
