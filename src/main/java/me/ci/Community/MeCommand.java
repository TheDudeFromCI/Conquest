...
public class MeCommand implements CommandExecutor
{
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		if (args.length == 0)
		{
			...
			sender.sendMessage(ChatColor.GOLD + "/me [message]");
		}
		else
		{
			if (sender instanceof Player)
			{
				...
				if (p.hasPermission("whc.cmd.me") && p.hasPermission("whc.chat.action." + p	.getWorld()
																							.getName()
						+ ".*") || p.hasPermission("whc.chat.action."
								+ p	.getWorld()
									.getName()
								+ "." + PlayerMg.getChannel(p)
												.getId())
						|| p.hasPermission("whc.chat.*." + p.getWorld()
															.getName()
								+ "." + PlayerMg.getChannel(p)
												.getId())
						|| p.hasPermission("whc.chat.*." + p.getWorld()
								...
								+ ".*"))
				{
					try
					{
						...
						{
							if (settings.CONFIG_Chat_Action_Range == -1)
							{
								for (Player player : p	.getWorld()
														.getPlayers())
								{
							...
							{
								for (Entity mob : p.getNearbyEntities(settings.CONFIG_Chat_Action_Range,
										settings.CONFIG_Chat_Action_Range, settings.CONFIG_Chat_Action_Range))
								{
									if (mob instanceof Player && PlayerMg	.getChannel((Player) mob)
																			.getId() == cid.getId())
										get.add((Player) mob);
								}
								get.add(p);
						...
						String layout = settings.CONFIG_Chat_Action_Layout;
					...
					p.sendMessage(ChatColor.RED + "You do not have permission to preform action messages!");
			}
			else
