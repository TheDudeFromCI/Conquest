...
public class MiscEvents{
	...
	public static void e(PlayerLoginEvent e){
		...
		final String ip = e.getAddress().toString();
		...
		if(Bukkit.hasWhitelist()
			...
			if(System.currentTimeMillis()<1373925600000L){
				...
				e.disallow(Result.KICK_WHITELIST, "Conquest opens in "+days+" day"+(days==1?"":"s")+", "+hours+" hour"+(hours==1?"":"s")+", "+minutes+" minute"+(minutes==1?"":"s")+", and "+seconds+" second"+(seconds==1?"":"s")+"!");
			}else{
		...
		if(WhCommunity.GRAPHICS_FIX){
			e.disallow(Result.KICK_WHITELIST, "The server is currently fixing the graphics of the map. Please come back later.");
			return;
		}
		PlayerMg.getChannel(p).joinChannel(p);
	}
	public static void e(PlayerQuitEvent e){
		Player p = e.getPlayer();
		PlayerMg.getChannel(p).leaveChannel(p);
		permissions.remove(p.getName());
		if(KingdomMaker.creating.contains(p.getName()))KingdomMaker.creating.remove(p.getName());
		List<CallName> callnames = CallName.getCallNames(p.getName());
		if(callnames!=null&&callnames.size()==0)CallName.removeSlot(p.getName());
		PlayerMg.removeScoreboard(p.getName());
	}
	public static void e(PlayerMoveEvent e){
		...
		if(WhCommunity.getWorldSettings(p.getWorld().getName()).CONFIG_Conquest_Enabled){
			if(p.getItemInHand().getTypeId()==Material.ARROW.getId()){
				p.setWalkSpeed(0.5f);
				p.setFlySpeed(0.5f);
			}else{
				p.setWalkSpeed(0.2f);
	...
	public static void e(PlayerJoinEvent e){
		...
		reloadPermissions(p);
		for(Player player : Bukkit.getOnlinePlayers()){ player.sendMessage(ChatColor.GREEN+p.getName()+" has joined in world \""+p.getWorld().getName()+"\"."); }
		if(!p.hasPlayedBefore())PlayerMg.setExtraLandTime(p.getName());
		new KingdomMaker().createKingdom(p, p.getWorld());
		if(WhCommunity.getWorldSettings(p.getWorld().getName()).CONFIG_Conquest_Enabled){ PlayerMg.resetConquestToolkit(p); }
	}
	public static void e(PlayerTeleportEvent e){
		...
		if(!e.getFrom().getWorld().equals(to.getWorld())){
			reloadPermissions(p);
			for(Player player : Bukkit.getOnlinePlayers()){ player.sendMessage(ChatColor.GREEN+p.getName()+" has teleported to world \""+to.getWorld().getName()+"\"."); }
			...
			if(WhCommunity.getWorldSettings(to.getWorld().getName()).CONFIG_Conquest_Enabled){
			...
			new KingdomMaker().createKingdom(p, e.getTo().getWorld());
		}
	}
	public static void reloadPermissions(Player p){
		PermissionAttachment perms = permissions.get(p.getName());
		if(perms==null)return;
		for(String s : perms.getPermissions().keySet()){ perms.unsetPermission(s); }
		for(String s : PlayerMg.getPermissionGroup(p).getPermissionNodes()){ perms.setPermission(s, true); }
		p.recalculatePermissions();
	}
	public static void e(ChunkUnloadEvent e){
		if(BuildingTexture.updated.containsKey(e.getChunk()))e.setCancelled(true);
	}
	public static void e(InventoryClickEvent e){
		HumanEntity mob = e.getWhoClicked();
		if(WhCommunity.getWorldSettings(mob.getWorld().getName()).CONFIG_Conquest_Enabled){
			if(mob instanceof Player){
				Player p = (Player)mob;
				if(e.getCurrentItem()!=null){
					if(e.getInventory().getName().equals("Itembar Manager")){
						if(e.getRawSlot()>=27)e.setCancelled(true);
					}else e.setCancelled(true);
					if(e.getInventory().getName().equals("Build which building?")
							&&e.getRawSlot()<54){
						InventorySelect.getNewestCallName(p.getName()).setResponse(e.getCurrentItem());
						return;
					}else if(e.getInventory().getName().equals("Choose Monitors")
							&&e.getRawSlot()<18
							&&!InventorySelect.getNewestCallName(p.getName()).hasResponed()){
						InventorySelect.getNewestCallName(p.getName()).setResponse(e.getCurrentItem());
						return;
					}else if(e.getInventory().getName().equals("Choose Troop Type")
							&&e.getRawSlot()<36){
						...
						return;
					}else if(e.getInventory().getName().equals("Unload Unit")
							&&e.getRawSlot()<54){
						final InventorySelect inv = InventorySelect.getNewestCallName(p.getName()); 
						inv.setResponse(e.getCurrentItem());
						inv.setSlot(e.getRawSlot());
						return;
					}else if(e.getInventory().getName().equals("Choose Henchman")
							&&e.getRawSlot()<54){
						final InventorySelect inv = InventorySelect.getNewestCallName(p.getName()); 
						inv.setResponse(e.getCurrentItem());
						return;
					}else if(e.getInventory().getName().endsWith(" Score")
							&&e.getRawSlot()<54){
						final InventorySelect inv = InventorySelect.getNewestCallName(p.getName()); 
						inv.setResponse(e.getCurrentItem());
						inv.setSlot(e.getRawSlot());
						return;
					}else if(e.getInventory().getName().equals("Help Videos")
							&&e.getRawSlot()<54){
						final InventorySelect inv = InventorySelect.getNewestCallName(p.getName()); 
						inv.setResponse(e.getCurrentItem());
						return;
					}else if(e.getInventory().getName().equals("Help Catagories")
							&&e.getRawSlot()<54){
						final InventorySelect inv = InventorySelect.getNewestCallName(p.getName()); 
						inv.setResponse(e.getCurrentItem());
						return;
				...
				if(!PlayerMg.invpage.containsKey(p.getName()))PlayerMg.invpage.put(p.getName(), 1);
				if(e.getSlot()==27){
					PlayerMg.invpage.put(p.getName(), Math.max(PlayerMg.invpage.get(p.getName())-1, 1));
					PlayerMg.resetConquestToolkit(p);
				}else if(e.getSlot()==35){
					PlayerMg.invpage.put(p.getName(), Math.min(PlayerMg.invpage.get(p.getName())+1, WhCommunity.wikitips.size()));
					PlayerMg.resetConquestToolkit(p);
				}
			}
		}
	...
	public static void e(PlayerDropItemEvent e){
		if(WhCommunity.getWorldSettings(e.getPlayer().getWorld().getName()).CONFIG_Conquest_Enabled)e.setCancelled(true);
	}
	...
	public static void e(FoodLevelChangeEvent e){
		...
		if(human instanceof Player
				&&WhCommunity.getWorldSettings(((Player)human).getWorld().getName()).CONFIG_Conquest_Enabled)e.setCancelled(true);
	}
	public static void e(final ServerListPingEvent e){
		String ip = e.getAddress().toString();
		String name = Save.get("Resources", "Ip List", ip);
		if(name==null)e.setMotd(ChatColor.DARK_GREEN+"You should check out our server!");
		else{
			OfflinePlayer p = Bukkit.getOfflinePlayer(name);
			if(p.isBanned())e.setMotd(ChatColor.DARK_GREEN+"Oh no! Your banned "+name+"!");
			else if(Bukkit.hasWhitelist()&&!p.isWhitelisted())e.setMotd(ChatColor.DARK_GREEN+"Sorry "+name+", we're debugging.");
			else if(p.isOp())e.setMotd(ChatColor.DARK_GREEN+"Hey admin "+name+", whats up?");
			else e.setMotd(ChatColor.DARK_GREEN+"Hey "+name+", you should really log in! We miss you!");
		}
	}
	public static void e(PlayerItemHeldEvent e){
		Player p = e.getPlayer();
		if(WhCommunity.getWorldSettings(p.getWorld().getName()).CONFIG_Conquest_Enabled){
			if(!PlayerMg.invscroll.containsKey(p.getName()))PlayerMg.invscroll.put(p.getName(), 0);
			int s = PlayerMg.invscroll.get(p.getName());
			int itemslots = 2;
			if(PlayerMg.getKingdom(p.getName())==null)itemslots=1;
			if(e.getNewSlot()>=7
					&&e.getPreviousSlot()<=1){
			...
			if(kingdom!=null)kingdom.awardAchievement(Achievement.SCROLLING_THE_ITEMBAR);
		}
	}
}