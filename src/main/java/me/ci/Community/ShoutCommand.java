...
public class ShoutCommand implements CommandExecutor
{
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		if (args.length == 0)
		{
			...
			sender.sendMessage(ChatColor.GOLD + "/shout [message]");
		}
		else
		{
			if (sender instanceof Player)
			{
				...
				WorldSettings settings = WhCommunity.getWorldSettings(p	.getWorld()
																		.getName());
				if (p.hasPermission("whc.cmd.shout") && p.hasPermission("whc.chat.shout." + p	.getWorld()
																								.getName()
						+ ".*") || p.hasPermission("whc.chat.shout."
								+ p	.getWorld()
									.getName()
								...
								+ ".*"))
				{
					try
					{
						...
						{
							if (settings.CONFIG_Chat_Shout_Range == -1)
							{
							...
							{
								for (Entity mob : p.getNearbyEntities(settings.CONFIG_Chat_Shout_Range,
										settings.CONFIG_Chat_Shout_Range, settings.CONFIG_Chat_Shout_Range))
								{
						...
						String layout = settings.CONFIG_Chat_Shout_Layout;
					...
					p.sendMessage(ChatColor.RED + "You do not have permission to shout messages!");
			}
			else
