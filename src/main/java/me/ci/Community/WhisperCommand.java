	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		...
		else
		{
			if (sender instanceof Player)
			{
				...
				if (p.hasPermission("whc.cmd.whisper") && p.hasPermission("whc.chat.whisper." + p	.getWorld()
																									.getName()
						+ ".*") || p.hasPermission("whc.chat.whisper."
								+ p	.getWorld()
									.getName()
								...
								+ ".*"))
				{
					try
					{
						...
						if (WhCommunity.CONFIG_Local_Chat)
						{
							...
							{
								for (Entity mob : p.getNearbyEntities(settings.CONFIG_Chat_Whisper_Range,
										settings.CONFIG_Chat_Whisper_Range, settings.CONFIG_Chat_Whisper_Range))
								{
					...
					p.sendMessage(ChatColor.RED + "You do not have permission to whisper messages!");
			}
			else
