...
public class Blacksmith extends Building implements ResourceHolder, VillagerWorkable{
	private int arms;
	private int workers;
	public Blacksmith(Chunk nwc, Kingdom kingdom){
		super(nwc, kingdom);
		if(WhCommunity.debug)WhCommunity.printDebug();
	}
	public Blacksmith(Kingdom kingdom, String code, Chunk nwc, long id){
	...
	public ResourceType[] getResources(){
		if(WhCommunity.debug)WhCommunity.printDebug();
		return new ResourceType[]{ResourceType.ARMS};
	}
	public double[] getResourcePercents(){
		if(WhCommunity.debug)WhCommunity.printDebug();
		return new double[]{arms/(double)getMaxResourceLevel()};
	}
	public void setResource(ResourceType type, int amount){
		if(WhCommunity.debug)WhCommunity.printDebug();
		if(type==ResourceType.ARMS)arms=amount;
	}
	public int getResourceLevel(ResourceType type){
		if(WhCommunity.debug)WhCommunity.printDebug();
		if(type==ResourceType.ARMS)return arms;
		return 0;
	}
	...
	public String[] getInfo(){
		...
		s[1]=ChatColor.DARK_AQUA+"Building Name .......... "+ChatColor.GRAY+"Blacksmith";
