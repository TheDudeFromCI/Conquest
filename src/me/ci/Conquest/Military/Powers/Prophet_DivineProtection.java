package me.ci.Conquest.Military.Powers;

import org.bukkit.ChatColor;
import org.bukkit.Location;
import org.bukkit.entity.Player;

import me.ci.Community.PlayerMg;
import me.ci.Community.Plot;
import me.ci.Community.PlotMg;
import me.ci.Conquest.Buildings.Main.Army;
import me.ci.Conquest.KingdomManagment.Kingdom;
import me.ci.Conquest.Military.PowerDetails;

public class Prophet_DivineProtection implements PowerDetails{
	private static final int stamina = 10;
	public void run(final Player p, final Location click, final Army troop){
		final Kingdom kingdom = PlayerMg.getKingdom(p.getName());
		final Plot plot = PlotMg.getPlotAt(click.getChunk());
		if(!troop.isNullified()){
			if(troop.getStamina()>=stamina){
				if(plot.isKingdomPlot()){
					if(plot.getKingdom()==kingdom){
						if(!(plot.getBuilding() instanceof Army)){
							if(Math.abs(troop.getNorthWestCourner().getX()-plot.getX())<=5
									&&Math.abs(troop.getNorthWestCourner().getZ()-plot.getZ())<=5){
								if(!plot.getBuilding().hasDivineProtection()){
									plot.getBuilding().divineProtection();
									troop.setStamina(troop.getStamina()-stamina);
									p.sendMessage(ChatColor.GREEN+"Building under divine protection.");
								}else{
									p.sendMessage(ChatColor.RED+"This building already has divine protection!");
									explain(p);
								}
							}else{
								p.sendMessage(ChatColor.RED+"That target is too far away!");
								explain(p);
							}
						}else{
							p.sendMessage(ChatColor.RED+"This unit can only use divine protection on buildings!");
							explain(p);
						}
					}else{
						p.sendMessage(ChatColor.RED+"This is not your kingdom!");
						explain(p);
					}
				}else{
					p.sendMessage(ChatColor.RED+"There is no kingdom here!");
					explain(p);
				}
			}else{
				p.sendMessage(ChatColor.RED+"This unit doesn't have enough stamina to use divine protection!");
				explain(p);
			}
		}else{
			p.sendMessage(ChatColor.RED+"This unit is nullified!");
			explain(p);
		}
	}
	private static void explain(final Player p){ p.sendMessage(ChatColor.GRAY+"To use this power, click on an ally troop unit and it will be given divine protection, so the next attack they recive is completly blocked."); }
}