...
public class GetBuildingLevel extends Action{
	public GetBuildingLevel(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(((BuildingClass)args.getInputs()[0]).getState().getLevel()));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}