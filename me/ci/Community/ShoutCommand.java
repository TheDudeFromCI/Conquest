	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		if(args.length==0){
			...
			if(sender instanceof Player){
				...
				if(p.hasPermission("whc.cmd.shout")
						&&p.hasPermission("whc.chat.shout."+p.getWorld().getName()+".*")
						||p.hasPermission("whc.chat.shout."+p.getWorld().getName()+"."+PlayerMg.getChannel(p).getId())
					...
					try{
						...
						if(WhCommunity.CONFIG_Local_Chat){
							if(settings.CONFIG_Chat_Shout_Range==-1){
								...
								for(Entity mob : p.getNearbyEntities(settings.CONFIG_Chat_Shout_Range, settings.CONFIG_Chat_Shout_Range, settings.CONFIG_Chat_Shout_Range)){
				...
				}else p.sendMessage(ChatColor.RED+"You do not have permission to shout messages!");
