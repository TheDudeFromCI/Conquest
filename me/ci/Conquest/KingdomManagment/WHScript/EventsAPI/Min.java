...
public class Min extends Action{
	public Min(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(Math.min(((NumberClass)args.getInputs()[0]).getState(), ((NumberClass)args.getInputs()[1]).getState())));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}