...
public class GetKingdom extends Action{
	public GetKingdom(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			if(cl.getPrimitiveType()!=PrimitiveType.KINGDOM)throw new Exception("Incorrect variable type: "+cl.getTypeName()+".");
			cl.setValueFromString(whs.getKingdom().getName());
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}