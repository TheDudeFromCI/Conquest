...
import me.ci.Conquest.KingdomManagment.WHScript.WraithavenScript.BuildingClass;
...
public class IsBuildingAnArmy extends Action{
	public IsBuildingAnArmy(final WraithavenScript whs, final ActionLineFormat format, final int line){
		...
		try{
			...
			if(args.getInputs()[0].getPrimitiveType()!=PrimitiveType.BUILDING)throw new Exception("Incorrect variable type: "+args.getInputs()[0].getPrimitiveType()+".");
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(((BuildingClass)args.getInputs()[0]).getState() instanceof Army));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}