...
public class Modulus extends Action{
	public Modulus(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(((NumberClass)args.getInputs()[0]).getState()%((NumberClass)args.getInputs()[1]).getState()));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}