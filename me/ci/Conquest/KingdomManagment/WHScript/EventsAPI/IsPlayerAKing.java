...
public class IsPlayerAKing extends Action{
	public IsPlayerAKing(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(PlayerMg.getKingdom(((PlayerClass)cl).getState().getName()).isKing()));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}