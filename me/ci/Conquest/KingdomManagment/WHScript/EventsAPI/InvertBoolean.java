...
public class InvertBoolean extends Action{
	public InvertBoolean(final WraithavenScript whs, final ActionLineFormat format, final int line){
		...
		try{
			...
			if(args.getInputs()[0].getPrimitiveType()!=PrimitiveType.BOOLEAN)throw new Exception("Incorrect variable type: "+args.getInputs()[0].getPrimitiveType()+".");
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(!((BooleanClass)cl).getState()));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}