...
public class GetBuilding extends Action{
	public GetBuilding(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			if(cl.getPrimitiveType()!=PrimitiveType.BUILDING)throw new Exception("Incorrect variable type: "+cl.getTypeName()+".");
			cl.setValueFromString(whs.getKingdom().getName()+","+whs.getBuilding().getId());
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}