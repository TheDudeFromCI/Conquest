...
public class Power extends Action{
	public Power(final WraithavenScript whs, final ActionLineFormat format, final int line){
	...
	public void run(){
		try{
			...
			cl.setValueFromString(String.valueOf(Math.pow(((NumberClass)args.getInputs()[0]).getState(), ((NumberClass)args.getInputs()[1]).getState())));
		}catch(Exception exception){ System.err.println("Error parsing event code! Line: "+line+" is broken.\n"+exception.getMessage()); }
	}
}