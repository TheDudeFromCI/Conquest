...
public enum HenchmenType{
	BOUNTY_HUNTER(4, "BountyHunter", 4000, 6000, new BountyHunter(), Material.IRON_FENCE),
	TINKER_GNOME(5, "TinkerGnome", 5000, 7000, new TinkerGnome(), Material.ANVIL),
	COMMANDER(9, "Commander", 7000, 9000, new Commander(), Material.GOLD_HELMET),
	...
	public boolean use(final Player p, final Plot plot){ return henchman.use(p, plot); }
	public void explain(final Player p){ henchman.explain(p); }
	public Material getMaterial(){ return material; }
	public static HenchmenType getById(final int id){
		for(HenchmenType men : values()){ if(men.getId()==id)return men; }
		return null;
	}
	public static HenchmenType getByName(final String name){
		for(HenchmenType men : values()){ if(men.getName().equalsIgnoreCase(name))return men; }
		return null;
	}
}