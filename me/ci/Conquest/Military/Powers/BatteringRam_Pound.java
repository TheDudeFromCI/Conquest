...
public class BatteringRam_Pound implements PowerDetails{
	...
	public void run(final Player p, final Location click, final Army troop){
		...
		if(!troop.isNullified()){
			if(troop.getStamina()>=stamina){
							...
							if(plot.getKingdom().isOwnerOnline()){
								if(!(plot.getBuilding() instanceof Army)){
									...
									if(troop.isTargetAttackable(true)){
										troop.usePower(plot.getKingdom(), "Your "+plot.getBuilding().getType().toString().toLowerCase()+" was hit by a battering ram!");
										troop.attack(2.0, false, false, true);
										troop.setStamina(troop.getStamina()-stamina);
										p.sendMessage(ChatColor.GREEN+"Building damaged.");
									}else{
									...
									p.sendMessage(ChatColor.RED+"You can only use this ability against buildings!");
									explain(p);
								}
							}else{
				...
				p.sendMessage(ChatColor.RED+"This unit doesn't have enough stamina to use pound!");
				explain(p);
			}
		}else{
	...
	private static void explain(final Player p){ p.sendMessage(ChatColor.GRAY+"To use this power, click on an enemy building. An attack dealing 200% damage will be preformed on it."); }
}